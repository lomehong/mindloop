...
传输按配置自动选择：command 走 stdio，url 走 streamable HTTP。`,
	...
	cmd.AddCommand(
		c.newMCPServeCmd(),
		c.newMCPListCmd(identityP),
		c.newMCPToolsCmd(identityP, timeoutP),
		c.newMCPCallCmd(identityP, timeoutP),
		c.newMCPAddCmd(identityP),
		c.newMCPRemoveCmd(identityP),
	)
	return cmd
}

func (c *CLI) newMCPListCmd(identityP string) *cobra.Command {
...
func (c *CLI) newMCPToolsCmd(identityP string, timeout time.Duration) *cobra.Command {
	return &cobra.Command{
		Use:   "tools <服务器名>",
...
func (c *CLI) newMCPCallCmd(identityP string, timeout time.Duration) *cobra.Command {
	return &cobra.Command{
...
func (c *CLI) newMCPAddCmd(identityP string) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := writeMCPConfig(writeFile, cfg); err != nil {
				return c.fail(err)
			}
...
func (c *CLI) newMCPRemoveCmd(identityP string) *cobra.Command {
	return &cobra.Command{
		Use:   "remove <名字>",
...
// writeMCPConfig 把合并后的配置写回 mcp.json。序列化失败必须上报
// ——静默丢一次写入等于让用户的 add/remove 凭空消失。
func writeMCPConfig(path string, cfg mcp.Config) error {
	data, err := json.MarshalIndent(cfg, "", "  ")
	if err != nil {
		return fmt.Errorf("mcp: 序列化 %s: %w", path, err)
	}
