...
// monolith 实现 Thinker。回退层级留在实例状态里：调度器保证同一
// 时刻至多一次唤醒在跑（busy 标志），层级无需并发保护。
...
func (m *monolith) Wake(ctx context.Context, w Wake) Outcome {
	...
	class, summary := m.classify(res, err)
	policy := backoffOrDefault(m.opts.Backoff)
	m.level = policy.Escalate(m.level, class, reactive)
...
// 归类 ClassWork。
func (m *monolith) classify(res runner.Result, err error) (WakeClass, string) {
	if err != nil {
	...
	if err := m.opts.Timeline.Append(context.Background(), s); err != nil {
		return ClassThoughtOnly, "结论落盘失败"
	}
	return ClassWork, fmt.Sprintf("完成一步（%d 轮）: %s", res.Iterations, oneLine(final, 80))
}
